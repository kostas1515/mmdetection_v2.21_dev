...
class FCNNormMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(FCNNormMaskHead, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.upsample is not None:
            ...
            if self.upsample_method == 'deconv':
                x = self.relu(x)
        mask_pred = F.conv2d(20 * F.normalize(x), F.normalize(self.conv_logits.weight), self.conv_logits.bias)
        return mask_pred
    ...
    def get_targets(self, sampling_results, gt_masks, rcnn_train_cfg):
        ...
        # print(['gt_masks:', [len(gt_masks_a) for gt_masks_a in gt_masks]])
        mask_targets = mask_target(pos_proposals, pos_assigned_gt_inds,
                                   gt_masks, rcnn_train_cfg)
        # torch.cuda.empty_cache()
        return mask_targets

    def get_targets_for_nms_sampling(self, pos_proposals, pos_assigned_gt_inds, gt_masks, rcnn_train_cfg):
        mask_targets = mask_target(pos_proposals, pos_assigned_gt_inds,
                                   gt_masks, rcnn_train_cfg)
        return mask_targets
